...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [workshopPlugin],
    });

    await request(server).get('/api/workshop/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/workshop/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/workshop/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/workshop/todos/${createdTodoItem.id}`)
